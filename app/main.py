...
sla = mid.selectbox(
    "SLA Tier",
    ["All"] + sorted(df["sla_tier"].dropna().unique().tolist()),
    key="filter_sla",
)
...
if filtered.empty:
    ...
    # Use a FORM to avoid duplicate buttons on reruns/branches
    with st.form(key="reco_form"):
        selected_id = st.selectbox(
            "Choose a circuit",
            filtered["circuit_id"].tolist(),
            key="circuit_selector",
        )

        submitted = st.form_submit_button(
            "Generate AI Recommendation",
            key="btn_gen_reco_submit",
        )
